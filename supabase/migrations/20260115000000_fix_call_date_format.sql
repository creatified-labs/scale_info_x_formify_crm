-- Fix: Convert call_date from timestamp to date format
-- This ensures "Calls on this day" filtering works correctly

-- First, let's check if call_date is storing timestamps instead of dates
-- and fix any existing records
UPDATE public.call_logs
SET call_date = DATE(call_date)
WHERE call_date::text LIKE '%:%';

-- Now update the sync function to use DATE() when storing call_date
...
BEGIN
  ...
  IF (TG_OP = 'INSERT') THEN
    INSERT INTO public.call_logs (
      ...
      DATE(NEW.start_time),  -- FIX: Cast to DATE
  ...
  IF (TG_OP = 'UPDATE') THEN
    ...
    SET
      ...
      call_date = DATE(NEW.start_time),  -- FIX: Cast to DATE
