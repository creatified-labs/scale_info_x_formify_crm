...
interface EmbedOption3Props {
  eventType: EventType & { branding_hide_badge?: boolean; user_timezone?: string };
}

export const EmbedOption3 = ({ eventType }: EmbedOption3Props) => {
  ...
  const allQuestionsAnswered = useMemo(() => {
    if (!name.trim() || !emailIsValid) return false;
    
    for (const question of questions) {
      const answer = questionAnswers[question.id];
      if (question.required && (!answer || answer.toString().trim().length === 0)) {
        return false;
      }
    }
    return true;
  }, [name, emailIsValid, questions, questionAnswers]);

  useEffect(() => {
    if (allQuestionsAnswered && calendarSectionRef.current) {
          ...
          <div className="space-y-6">
            ...
            <div 
              ...
              className={cn(
                ...
                !allQuestionsAnswered && "opacity-50 pointer-events-none"
              )}
            >
              <div className="border-t pt-6">
                <h2 className="text-lg font-semibold mb-4">
                  {allQuestionsAnswered ? "Select Your Preferred Time" : "Complete the form above to select a time"}
                </h2>
                {allQuestionsAnswered && (
                    ...
                    <div>
                      ...
                      <BookingCalendar
                        form={(() => {
                          ...
                          console.log('📅 [EmbedOption3] BookingCalendar form data:', {
